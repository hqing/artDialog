...
(function() {
	...
	isIE6 = !+'\v1' && ([/MSIE (\d)\.0/i.exec(navigator.userAgent)][0][1] == 6),
	Each = function(a, b) {
	...
	newBox = function() {
		...
		$.zIndex = function(o) {
			var x = o ? o : ui_dialog;
			x.style.zIndex = ++z;
			if (isIE6) ui_dialog_wrap.style.zIndex = ++z;//IE6叠加高度受具有绝对或者相对定位的父元素z-index影响
			return $;
		};
		...
		$.close = function(f) {
			...
			function _close(){
				...
				ui_dialog.style.cssText = ui_title_text.innerHTML = ui_content.innerHTML = ui_btns.innerHTML = '';//清空内容自动隐藏对话框
		...
		$.html = function(title, url, content, yesFn, noFn, yesText, noText, style, closeFn, time, yesClose) {
			...
			if (content) {
				ui_content.innerHTML = '<span class="ui_dialog_icon"></span>' + content;//写入内容
			} else {
				var g = $ce('iframe');
				g.src = url;
				addClass(ui_content, 'ui_iframe');
				ui_content.appendChild(g);//写入嵌入式窗口
			};
		...
		$.align = function(width, height, x, y, fixed, lock, c) {
			...
			if (fixed) {
				addClass(document.getElementsByTagName('html')[0], 'ui_ie6_fixed');
				...
				maxY = C.ddh - C.h;
				//黄金比例垂直居中
				var centerY = (C.ddh * 0.382 - C.h / 2 > 0) ?  C.ddh * 0.382 - C.h / 2 : maxY / 2;
			} else {
				minX = C.sl;
				maxX = C.ddw + minX - C.w;
				centerX = maxX / 2;
	...
	ie6png = function(){
		if (ghead) {
			...
			script.src = path + '/iepngfix/iepngfix_tilebg.js';//路径设置
	...
	domReady(function(){
		...
		if (isIE6) {
			...
			if (ie6PngRepair) {
				...
				//png背景半透明支持
				addStyle('.ui_resize, .ui_ie6_select_mask { display:none; } td.ui_border, td.ui_title_wrap *, .ui_dialog_icon  { behavior: url("' +path+ '/iepngfix/iepngfix.htc")}');//iepngfix.htc的路径与带有png背景的CSS选择器设置(没有写上按钮，因为实际应用中有点小问题)
			};
		} else {
			//预加载结构与背景图片
