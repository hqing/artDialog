(function() {
	...
	newBox = function() {
		...
		W.close = function() {
			...
			cTitleText.innerHTML = cContentWrap.innerHTML = cBottom.innerHTML = '';//清空内容
			removeClass(cContentWrap, 'cIframeWrap');//移除嵌入框架专属样式
			removeClass(dWrap, 'dFixed');//移除静止定位属性
			bTable.className = '';//移除风格属性
			var h = document.getElementsByTagName('html')[0];
			if (h.className.indexOf('mesLock') > -1){
				W.ai(dOverlay, 1, function(){
					removeClass(h, 'mesLock');//移除锁屏样式
				});
			} else {
		...
		W.html = function(t, a, b, y, n, c, d, s, w, h) {
			...
			if (y) {
				...
				yn.className = 'YN';
				yn.innerHTML = b || '';
				cBottom.appendChild(f);
				//cTbody.appendChild(cBottom);
				//yn.appendChild(f);
				if (n) {
					no.onclick = function() {
						n();
		...
		//透明渐变
		W.alpha = function(obj, endInt){
			/*if(document.all){
				obj.filters.alpha.opacity += 20;
				if(obj.filters.alpha.opacity < endInt) setTimeout(function(){W.alpha(obj,endInt)},5);
			}else{
				var al=parseFloat(obj.style.opacity);al += 0.20;
				obj.style.opacity=al;
				if(al < (endInt/100)) setTimeout(function(){W.alpha(obj,endInt)},5);
			};*/
			...
			setTimeout(function(){W.alpha(obj,endInt)},5);
		};
		
		W.ai = function(o, k , f, x){
			if(!x) i = k;
			s = 0.05;
			s = (k == 0) ? s : -s;
			i += s;
			...
			if (i > 0 && i < 1) {
				setTimeout(function(){W.ai(o, k, f, i)}, 5);
			} else if(f) {
				f();
			};
		};
